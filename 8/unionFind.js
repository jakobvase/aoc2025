...
function run(coords, startAt) {
  ...
  /** @type {Record<symbol, {value: [string, string, string]}>} */
  let forest = coords.reduce(
    unionFind.makeSet,
    unionFind.makeForest((v) => JSON.stringify(v))
  );
  console.log("Done. Adding edges.");
  // console.dir(forest, { depth: null });
  // console.dir(Object.entries(forest), { depth: null });
  let lastEdge;
  // console.log(unionFind.countRoots(forest));
  while (unionFind.countRoots(forest) > 1) {
    lastEdge = shortestCircuits.shift();
    // console.dir(lastEdge);
    if (lastEdge == null) {
      console.log("About to fail.");
      console.dir({ forest }, { depth: null });
    }
    forest = unionFind.union(
      forest,
      unionFind.valueToSet(forest, lastEdge[0]),
      unionFind.valueToSet(forest, lastEdge[1])
    );
  }
